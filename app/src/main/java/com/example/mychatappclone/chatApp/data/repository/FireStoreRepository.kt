package com.example.mychatappclone.chatApp.data.repository
...
import com.example.mychatappclone.chatApp.data.models.Message
import com.example.mychatappclone.chatApp.data.models.User
import com.example.mychatappclone.core.domain.UiState
import com.example.mychatappclone.chatApp.data.models.Chat
...
class FireStoreRepositoryImpl @Inject constructor(val fireStore: FirebaseFirestore, val firebaseAuth: FirebaseAuth,
                                                  private val context : Context
): FireStoreRepository {


    override fun setUser(user: User, phoneNumber: String) {
    ...
    override fun getUserData(phoneNumber: String, onUserDataReceived: (User?)-> Unit) {
        fireStore.collection("user")
            .document(phoneNumber)
            .get()
            .addOnSuccessListener {
    ...
    override fun getFileExtension(context: Context, uri : Uri?): String?{
    ...
    override fun checkUserPhone(phone: String) {
        fireStore.collection("user")
    ...
    override fun generateChatId(user1: String, user2: String): String {
        return if (user1 < user2) "${user1}-$user2" else "$user2-$user1"
    }

    override fun createOneOnOneChat( secondUserNumber: String) {
    ...
    override fun sendMessage(senderId: String, content: String, chatId: String) {
        val message = Message(
    ...
    override suspend fun checkChatExistence(secondPhoneNumber:String): Boolean? {
           val exists =
    ...
    override suspend fun checkUserExistence(userNumber : String): Boolean {
        val exists =
                fireStore.collection("user")
                    .document(userNumber)
                    .get()
    ...
    override fun getUserName():Flow<UiState<String>>{
        return callbackFlow {
    ...
    override fun addFriendNameToMyData(currentPhoneNumber: String, otherUserName: String){
    ...
    override suspend fun uploadImage(imageUri : Uri, context : Context):String? = withContext(Dispatchers.IO){
        try{
    ...
    override suspend fun getTheUploadedImage(userImageId: String): String? = withContext(Dispatchers.IO){
        try{
    ...
    override fun getUserImage(): Flow<UiState<String?>>{
        return flow {
            try {
                val user = fireStore.collection("user")
                    .document(userPhoneNumber())
    ...
    override suspend fun updateUserImage(uri: Uri?){
        uri?.let {
